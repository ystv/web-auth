...
-- Creating our people schema which will store personal data, roles, permissions and officerships
...
-- 6. people.officerships REFERENCES people.roles, people.officerships
-- 7. people.officership_members REFERENCES people.users, people.officerships
-- 8. people.officership_teams
-- 9. people.officership_team_member REFERENCES people.officership_teams people.officerships
--
...
CREATE TABLE IF NOT EXISTS people.users (
    ...
    ldap_username text UNIQUE,

    CONSTRAINT loginchk CHECK (login_type IN ('internal', 'sso', 'ldap'))
);
...
COMMENT ON COLUMN people.users.login_type IS
    'internal - uses pw in this table to authenticate. sso - uses external auth. ldap - uses LDAP for login.';
COMMENT ON COLUMN people.users.ldap_username IS
    'can be null but if filled then must be unique';
--
...
CREATE TABLE IF NOT EXISTS people.role_members (
...
CREATE TABLE IF NOT EXISTS people.permissions (
...
CREATE TABLE IF NOT EXISTS people.role_permissions (
...
--
-- people.officerships
CREATE TABLE IF NOT EXISTS people.officerships(
...
CREATE TABLE IF NOT EXISTS people.officership_members(
...
CREATE TABLE IF NOT EXISTS people.officership_teams(
...
CREATE TABLE IF NOT EXISTS people.officership_team_members(
...
-- web_auth contains both crowd apps and api tokens so is also critical to keep secure
CREATE SCHEMA IF NOT EXISTS web_auth;
-- We will create the tables in the following order
-- 1. web_auth.api_tokens
-- 2. web_auth.crowd_apps
--
-- web_auth.api_tokens
CREATE TABLE IF NOT EXISTS web_auth.api_tokens(
    token_id int GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    name text NOT NULL,
    description text,
    expiry timestamptz NOT NULL,
    user_id int NOT NULL REFERENCES people.users(user_id) ON UPDATE CASCADE ON DELETE CASCADE
);

--
-- web_auth.crowd_apps
CREATE TABLE IF NOT EXISTS web_auth.crowd_apps(
