...
func (s *Store) addToken(ctx context.Context, t1 Token) (Token, error) {
	...
	stmt, err := s.db.PrepareNamedContext(ctx, `INSERT INTO web_auth.api_tokens
    (token_id, name, description, expiry, user_id)
	VALUES (:token_id, :name, :description, :expiry, :user_id)
    RETURNING token_id, name, description, expiry, user_id`)
	if err != nil {
